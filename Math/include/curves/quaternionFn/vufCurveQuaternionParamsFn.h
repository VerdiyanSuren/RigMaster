...
namespace vufMath
{
	...
	public:
		...
		{			
			...
			V<T> l_tng;
			vufQuaternion_d l_quat;
			if (p_val <= m_quat_param_v[m_quat_indeces_v.front()])
			{
				l_tng	= p_curve.get_tangent_normalized_at(p_val);
			...
			if (p_val >= m_quat_param_v[m_quat_indeces_v.back()] )
			{
				l_tng = p_curve.get_tangent_normalized_at(p_val);
			...
			for (uint64_t i = 1; i < m_quaternion_a_v.size(); ++i)
			{
				...
				if (p_val >= m_quat_param_v[l_index_1] && p_val <= m_quat_param_v[l_index_2])
				{
					...
					{
						return (m_quaternion_a_v[l_index_1]  + m_quaternion_b_v[l_index_1]).get_normalized();
					}
		...
		// update param and quaternion. need param sorting after  
	...
	private:
		std::vector<uint32_t>			m_quat_indeces_v;
