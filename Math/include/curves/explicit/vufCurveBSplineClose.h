...
namespace vufMath
{
	...
	public:
		virtual ~vufCurveBSplineClose() {}
		VF_MATH_CURVE_DEFINE_CREATOR(vufCurveBSplineClose);
		VF_MATH_CURVE_DEFINE_TYPE_CATEGORY(k_close_bspline, k_bspline_category);
		...
		{
			std::shared_ptr< vufCurveBSplineClose > l_ptr = vufCurveBSplineClose::create();
		...
		virtual void		set_node_at(uint32_t p_index, const V<T>& p_vector) override
		{
			...
			*/
			...
			m_nodes_pos_v[p_index + m_indent_start] = p_vector - m_pos_offset;
			if (p_index < m_indent_end)
			{
				m_nodes_pos_v[p_index + m_indent_start + m_node_count] = p_vector - m_pos_offset;
				return;
			}
			if (p_index > m_node_count - 1 - m_indent_start)
			{
				m_nodes_pos_v[m_indent_start - (m_node_count - p_index)] = p_vector - m_pos_offset;
			}
		}
...
std::cout << "]" << std::endl;
	...
	public:
		...
		inline V<T>			get_pos_at_i(T p_t)		const
		{
			...
			if (p_t > 1.0)
			{
				return m_pos_offset + m_nodes_pos_v.back() + get_tangent_at_i(1) * (p_t - 1);
			}
			...
			for (int l_dgr = 0; l_dgr < CURVE_DEGREE + 1; ++l_dgr)
			{
				...
				l_res += m_nodes_pos_v[l_node_id] * m_n_v[l_interval_id][l_node_id].eval(p_t);
			}
			return l_res;
		}
...
#pragma region CAST SPECIALIZATION
	//specializations
	template<>
	std::shared_ptr<vufCurveBSplineClose <double, vufVector4, 1>>		vufCurveBSplineClose <double, vufVector4, 1>::as_close_bspline_mono()	const
	{
		return std::static_pointer_cast<vufCurveBSplineClose<double, vufVector4, 1>>(vufCurve<double, vufVector4>::m_this.lock());
	}
	template<>
	std::shared_ptr<vufCurveBSplineClose <double, vufVector4, 2>>		vufCurveBSplineClose <double, vufVector4, 2>::as_close_bspline_di()		const
	{
		return std::static_pointer_cast<vufCurveBSplineClose<double, vufVector4, 2>>(vufCurve<double, vufVector4>::m_this.lock());
	}
	template<>
	std::shared_ptr<vufCurveBSplineClose <double, vufVector4, 3>>		vufCurveBSplineClose <double, vufVector4, 3>::as_close_bspline_tri()	const
	{
		return std::static_pointer_cast<vufCurveBSplineClose<double, vufVector4, 3>>(vufCurve<double, vufVector4>::m_this.lock());
	}
#pragma endregion
#pragma region CLOSEST_POINTS
	template<>
	double		vufCurveBSplineClose <double, vufVector4, 1>::get_closest_point_param_i(const vufVector4<double>& p_point, double p_start, double p_end, uint32_t p_divisions, double p_percition) const
	{
	...
	template<>
	double		vufCurveBSplineClose <double, vufVector4, 2>::get_closest_point_param_i(const vufVector4<double>& p_point, double p_start, double p_end, uint32_t p_divisions, double p_percition) const
	{
		auto l_point = p_point - vufCurveExplicit<double, vufVector4>::m_pos_offset;
		double l_dist_min = (get_pos_no_offset_i(p_start) - l_point).length2();//std::numeric_limits<double>::max();
		double l_param = p_start;
	...
	template<>
	double		vufCurveBSplineClose <double, vufVector4, 3>::get_closest_point_param_i(const vufVector4<double>& p_point, double p_start, double p_end, uint32_t p_divisions, double p_percition) const
	{
