...
MStatus	vufCurveNoiseNode::initialize()
{
	...
	// 	enable
	g_enable_attr = l_numeric_attr_fn.create("enable", "ar", MFnNumericData::kBoolean, true, &l_status);
	...
	// amount x
	VF_RM_CREATE_STORABLE_NUMERIC_ATTR(g_amount_x_attr, "amountX", "ax", kDouble, 0.0);
	l_numeric_attr_fn.setChannelBox(true);
	l_status = addAttribute(g_amount_x_attr); CHECK_MSTATUS_AND_RETURN_IT(l_status);
	// amount y
	VF_RM_CREATE_STORABLE_NUMERIC_ATTR(g_amount_y_attr, "amountY", "ay", kDouble, 0.0);
	l_numeric_attr_fn.setChannelBox(true);
	l_status = addAttribute(g_amount_y_attr); CHECK_MSTATUS_AND_RETURN_IT(l_status);
	// amount x
	VF_RM_CREATE_STORABLE_NUMERIC_ATTR(g_amount_z_attr, "amountZ", "az", kDouble, 0.0);
	l_numeric_attr_fn.setChannelBox(true);
	...
	// scale x
	VF_RM_CREATE_STORABLE_NUMERIC_ATTR(g_scale_z_attr, "scaleZ", "az", kDouble, 1.0);
	l_numeric_attr_fn.setChannelBox(true);
	l_status = addAttribute(g_scale_z_attr); CHECK_MSTATUS_AND_RETURN_IT(l_status);

	// offset x
	VF_RM_CREATE_STORABLE_NUMERIC_ATTR(g_offset_x_attr, "offsetX", "ox", kDouble, 1.0);
	l_numeric_attr_fn.setChannelBox(true);
	l_status = addAttribute(g_offset_x_attr); CHECK_MSTATUS_AND_RETURN_IT(l_status);
	// offset y
	VF_RM_CREATE_STORABLE_NUMERIC_ATTR(g_offset_y_attr, "offsetY", "oy", kDouble, 1.0);
	l_numeric_attr_fn.setChannelBox(true);
	l_status = addAttribute(g_offset_y_attr); CHECK_MSTATUS_AND_RETURN_IT(l_status);
	// offset z
	VF_RM_CREATE_STORABLE_NUMERIC_ATTR(g_offset_z_attr, "offsetZ", "oz", kDouble, 1.0);
	l_numeric_attr_fn.setChannelBox(true);
...
MStatus	vufCurveNoiseNode::compute(const MPlug& p_plug, MDataBlock& p_data)
{
	if (p_plug == g_data_out_attr)
	{
		...
		l_crv->set_time_i(p_data.inputValue(g_speed_attr).asDouble());
		...
		l_crv->set_container(l_in_data->m_internal_data);
		return MS::kSuccess;
	}
