...
vufCurveScaleData::vufCurveScaleData() {}
vufCurveScaleData::~vufCurveScaleData() {}
MStatus 	vufCurveScaleData::readASCII(const MArgList& p_args, unsigned int& p_last_element)
{
	MStatus l_status;
	int l_type;
	l_type = p_args.asInt(p_last_element++, &l_status);
	m_internal_data = vufCurveScaleFn_4d::create(vufCurveScaleFnType(l_type));
...
MStatus 	vufCurveScaleData::readBinary(std::istream& p_in, unsigned int	p_length)
{
	int l_type;
...
MStatus 	vufCurveScaleData::writeASCII(std::ostream& p_out)
{
	if (m_internal_data == nullptr)
	{
		int l_type = (int)vufCurveScaleFnType::k_none;
...
MStatus 	vufCurveScaleData::writeBinary(std::ostream& p_out)
{
	if (m_internal_data == nullptr)
	{
		int l_type = (int)vufCurveScaleFnType::k_none;
