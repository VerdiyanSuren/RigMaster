...
vufCurveRebuildData::vufCurveRebuildData() {}
vufCurveRebuildData::~vufCurveRebuildData() {}
MStatus 	vufCurveRebuildData::readASCII(const MArgList& p_args, unsigned int& p_last_element)
{
	MStatus l_status;
	int l_type;
	l_type = p_args.asInt(p_last_element++, &l_status);
	m_internal_data = vufCurveRebuildFn_4d::create(vufCurveRebuildFnType(l_type));
...
MStatus 	vufCurveRebuildData::readBinary(std::istream& p_in, unsigned int	p_length)
{
	int l_type;
...
MStatus 	vufCurveRebuildData::writeASCII(std::ostream& p_out)
{
	if (m_internal_data == nullptr)
	{
		int l_type = (int)vufCurveRebuildFnType::k_none;
...
MStatus 	vufCurveRebuildData::writeBinary(std::ostream& p_out)
{
	if (m_internal_data == nullptr)
	{
		int l_type = (int)vufCurveRebuildFnType::k_none;
